...
begin
  if exists (
    ...
    with ranked as (
      select ctid, row_number() over (order by created_at asc nulls last, updated_at asc nulls last, ctid) as rn
...
create unique index if not exists photos_storage_path_uidx
